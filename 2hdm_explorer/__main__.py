...
def plot_MT(cat, bins, s=None):
    ...
    fig.savefig(f'MT_{cat}_{s}_{bins}.png')
...
def plot_MET(cat, bins, s=None):
    ...
    ax.set_yscale("log")
    ax.set_ylim([0.1, 90000])
    fig.savefig(f'MET_{cat}_{s}_{bins}.png')
...
def plot_efficiency_MT(cat, s):
    ...
    for i in s:
        ...
        all = list(output['MT1'].integrate('region', f'all_{cat}').values())[(i,)]
        sig = list(output['MT1'].integrate('region', f'cat_{cat}').values())[(i,)]
        plt.plot(list(range(0, int(len(all)*all_), int(all_))), [np.divide(sig[j], all[j]) for j in range(len(all))])
...
def plot_sensitivities(variables, cat):
    def calculate(variable):
        ...
        bkg = list(output[variable].integrate('region', f'cat_{cat}').values())[("bkg",)]
        results = []
        mH = []
        ma = []
        for s in list(datasets.keys()):
            if "bkg" in s: continue
            try:
                ...
                print(f"Warning: primary dataset {s} doesn't exist")
...
if __name__ == '__main__':
    ...
    for cat in ['0j', '1j', '01j']:
        ...
        plot_MET(cat, 600, s=("Pseudoscalar_mH-200_ma-100",
                              "Pseudoscalar_mH-300_ma-100",
                              "Pseudoscalar_mH-400_ma-100",
                              "Pseudoscalar_mH-500_ma-100",
                              "Pseudoscalar_mH-600_ma-100",

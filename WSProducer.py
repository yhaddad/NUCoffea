...
class WSProducer(ProcessorABC):
    ...
    def __init__(self, isMC, era=2017, sample="DY", do_syst=False, syst_var='', weight_syst=False, flag=False):
        self._flag = flag
        self.do_syst = do_syst
        self.era = era
        self.isMC = isMC
        self.sample = sample
        self.syst_var, self.syst_suffix = (syst_var, f'_sys_{syst_var}') if do_syst else ('', '')
    ...
    def postprocess(self, accumulator):
        for h, hist in accumulator.items():
            f = recreate(f'{self.sample}/{h}.root')
            f[h] = export1d(hist)
            f.close()
        return accumulator
